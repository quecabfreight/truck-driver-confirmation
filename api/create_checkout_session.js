...
export default async function handler(req, res) {
  ...
  try {
    ...
    const email = normalizeEmail(body.business_email || body.email);
    ...
    if (!email || !email.includes("@")) {
      return json(res, 400, {
        ok: false,
        error: "Missing valid approved broker email.",
      });
    }
    ...
    const origin =
      safe(req.headers.origin) ||
      safe(process.env.PUBLIC_SITE_URL) ||
    ...
    params.append("metadata[business_email]", email);
    params.append("metadata[email]", email);
    params.append("metadata[plan]", plan);

    params.append("subscription_data[metadata][business_email]", email);
    params.append("subscription_data[metadata][email]", email);
    params.append("subscription_data[metadata][plan]", plan);

    params.append(
      "success_url",
      `${origin}/billing-success.html?session_id={CHECKOUT_SESSION_ID}`
    );

    params.append("cancel_url", `${origin}/billing-cancel.html`);
